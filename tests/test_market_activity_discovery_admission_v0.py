...
class MarketActivityDiscoveryAdmissionV0Tests(unittest.TestCase):
    ...
    def test_success_path_freezes_t0_registers_analyzable_and_schedules_outcomes(self):
        ...
        self.assertEqual(result.cohort_member.snapshot_payload_sha256, result.preparation.snapshot_record.payload_sha256)
        self.assertEqual(
    ...
    def test_failed_t0_validation_can_be_preserved_without_silent_upgrade(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(database, "settings", SimpleNamespace(database_path=path)):
                ...
                episode = self._episode()
                baseline, mode, _activity = self._t0_inputs(as_of=110)
                _bad_baseline, _bad_mode, bad_activity = self._t0_inputs(as_of=109)
                with self.assertRaises(ValueError):
                    prepare_and_register_market_activity_episode_v0(
                        acquisition_run_key="RUN1",
                        episode_key=episode.episode_key,
                ...
                good_activity = self._t0_inputs(as_of=110)[2]
