...
from src.pumpswap_ready_scheduler import ReadyAssetScheduler, SchedulerAssetTelemetry
...
def _asset_label(asset: str) -> str:
    return f"{asset[:10]}…" if len(asset) > 10 else asset


def _concentration_asset_count(
    telemetry: tuple[SchedulerAssetTelemetry, ...], fraction: float
) -> int:
    waits = sorted(
        (item.dependency_wait_total_seconds for item in telemetry if item.dependency_wait_total_seconds > 0),
        reverse=True,
    )
    total = sum(waits)
    if total <= 0:
    ...
    running = 0.0
    for index, value in enumerate(waits, start=1):
        running += value
        if running >= target:
            return index
    return len(waits)
...
def _format_hot_assets(
    telemetry: tuple[SchedulerAssetTelemetry, ...],
    *,
    key: str,
    limit: int = 5,
) -> str:
    if key == "reservations":
        ordered = sorted(
            telemetry,
            key=lambda item: (-item.reservations, -item.max_outstanding_tickets, item.asset),
        )
        return " ".join(
            f"{_asset_label(item.asset)}:reservations={item.reservations},"
            f"max_outstanding={item.max_outstanding_tickets},max_waiting={item.max_waiting_jobs}"
    ...
    if key == "causal_wait":
        ordered = sorted(
            telemetry,
            key=lambda item: (-item.dependency_wait_total_seconds, -item.reservations, item.asset),
        )
        return " ".join(
            f"{_asset_label(item.asset)}:wait_total_ms={item.dependency_wait_total_seconds * 1000:.1f},"
            f"wait_p95_ms={item.dependency_wait_p95_seconds * 1000:.1f},"
            f"active_waiting={item.active_waiting_jobs}"
            for item in ordered[:limit]
            if item.dependency_wait_total_seconds > 0
        ) or "none"
    raise ValueError(f"unsupported hot-asset key: {key}")


async def run_smoke_v10(
    *,
    run_key: str,
    ...
    pumpswap_completed: asyncio.Queue[TimedPumpSwapCompletion] = asyncio.Queue()
    scheduler: ReadyAssetScheduler[TimedPumpSwapCompletion] = ReadyAssetScheduler()
    ...
    pending_pumpswap: dict[int, TimedPumpSwapCompletion] = {}
    ...
    async def pumpswap_dispatcher() -> None:
        ...
        while time.monotonic() < deadline:
            remaining = deadline - time.monotonic()
            try:
                timed = await asyncio.wait_for(pumpswap_completed.get(), timeout=min(0.5, remaining))
            except asyncio.TimeoutError:
                continue
            pending_pumpswap[timed.completed.sequence] = timed
            pumpswap_completed.task_done()
            while next_sequence in pending_pumpswap and time.monotonic() < deadline:
                timed = pending_pumpswap.pop(next_sequence)
                ...
                if (
                    ...
                    now = time.monotonic()
                    pumpswap_radar_wait_seconds.append(now - item.enqueued_monotonic)
                    pumpswap_pipeline_end_to_end_seconds.append(now - item.enqueued_monotonic)
                    next_sequence += 1
                    continue
                ...
                pumpswap_persist_to_reservation_seconds.append(
                    max(
                        0.0,
                        reservation.created_monotonic - timed.persistence_completed_monotonic,
                    )
                )
                ...
                scheduler.submit(timed, reservation)
                next_sequence += 1
        ...
        while time.monotonic() < deadline:
            ...
            try:
                ...
                pumpswap_ready_queue_wait_seconds.append(
                    max(0.0, started_service - work.ready_queue_entered_monotonic)
                )
                pumpswap_radar_wait_seconds.append(
                    started_service - item.enqueued_monotonic
                )

                result = await asyncio.to_thread(
                    evaluate_persisted_pumpswap_notification_for_radar_v4,
                    item.notification,
                    acquisition_run_key=run_key,
                    persist_result=item.persist_result,
                )
                ...
                pumpswap_history_read_seconds.append(telemetry.history_read_seconds)
                pumpswap_db_read_seconds.append(telemetry.db_read_seconds)
                pumpswap_detect_seconds.append(telemetry.detect_seconds)
                pumpswap_episode_assign_seconds.append(telemetry.episode_assign_seconds)
    ...
    ps_inflight = max(
        0, enqueued["pumpswap"] - persistence_completed["pumpswap"] - ps_ingress
    )
    ps_backlog = max(
    ...
    print(
        ...
        f"'pumpswap_inflight': {ps_inflight}, 'pumpswap_reorder': {ps_backlog}"
        "} "
    ...
    print(
        ...
        f"max_size={max_batch} configured_size={pump_batch_size} "
    ...
    print(
        f"pumpswap_radar_end_to_end_wait_ms "
        f"{_latency_summary_ms(pumpswap_radar_wait_seconds)}"
    )
    print(
        f"pumpswap_radar_service_time_ms "
        f"{_latency_summary_ms(pumpswap_radar_service_seconds)}"
    )
    ...
    print(
        f"pumpswap_ready_queue_wait_ms "
    ...
    print(
        f"pumpswap_ready_scheduler radar_workers={pumpswap_radar_workers} "
    ...
    print(
        "pumpswap_causal_asset_concentration "
    ...
    print(
        "pumpswap_hot_assets_by_causal_wait "
    ...
    print(
        "v10 is diagnostic-only. Pump ordered microbatch, PumpSwap persistence, per-asset FIFO, "
        "detector thresholds, causal clocks, replay semantics, episode semantics and provider policy "
        "are unchanged. PumpSwap radar workers remain 8. Jupiter/risk/12h remain blocked."
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Unified market latency smoke v10 diagnostic")
    ...
    print("Crypto Copy Trader — Unified Market Latency Smoke v10 Diagnostic")
