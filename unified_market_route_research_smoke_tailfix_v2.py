...
async def run_smoke_tailfix_v2(**kwargs) -> None:
    ...
    preserving same-token serialization. Tailfix v2 changes only the v52 hedged transport owner:
    endpoint calls live in one resolver-owned executor bounded by the already-configured
    max_concurrent_resolutions value, so a batch can publish its winner/deadline decision without
    retaining a v41 batch slot merely for losing-transport cleanup.
    """

    original_resolver = v52.DeadlineBoundedParallelHedgedResolverV52
    SharedTransportDeadlineResolverTailfixV2.last_instance = None
    v52.DeadlineBoundedParallelHedgedResolverV52 = (
        SharedTransportDeadlineResolverTailfixV2
    )
    ...
    try:
        ...
        v52.DeadlineBoundedParallelHedgedResolverV52 = original_resolver
        ...
        if resolver is None:
            ...
            if run_error is None:
                raise RuntimeError("tailfix v2 resolver was not installed")
            return
        ...
        print(
            "tailfix_v2_note=hedge decision availability releases the v41 batch slot immediately. "
            "Already-running loser transports drain only inside one shared executor whose worker "
            "count equals the inherited max_concurrent_resolutions ceiling; queued losers are "
            "cancelled. Cache, per-pool single-flight, hydration budget, as-of, reservation FIFO, "
            "detector, provider pacing and economics remain unchanged."
        )
