...
async def run_smoke(
    *,
    ...
    async def pump_worker() -> None:
        while time.monotonic() < deadline:
            ...
            try:
                ...
                continue
            try:
                pump_wait_seconds.append(time.monotonic() - item.enqueued_monotonic)
                result = await asyncio.to_thread(
                    process_pump_notification_for_radar,
                    item.notification,
                    acquisition_run_key=run_key,
                )
        ...
        while time.monotonic() < deadline:
            ...
            try:
                completed = await asyncio.wait_for(
                    pumpswap_completed.get(), timeout=min(0.5, remaining)
                )
            except asyncio.TimeoutError:
                continue
    ...
    tasks = [
        ...
        asyncio.create_task(pump_worker(), name="worker-pump"),
    ...
    pump_backlog = pump_queue.qsize()
    pumpswap_reorder_backlog = len(pending_pumpswap) + pumpswap_completed.qsize()
    pumpswap_inflight = max(
        0,
        enqueued["pumpswap"] - persistence_completed["pumpswap"] - pumpswap_ingress_backlog,
    )
    ...
    print(
        f"backlog_at_deadline={{'pump': {pump_backlog}, 'pumpswap_ingress': {pumpswap_ingress_backlog}, "
    ...
    print(f"pump_queue_wait_ms {_latency_summary_ms(pump_wait_seconds)}")
    ...
    print(
        "v3 preserves Pump ordering and PumpSwap radar ingress ordering while allowing bounded "
        "concurrent PumpSwap identity persistence. Execution/risk providers are not called."
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Unified market throughput smoke v3")
    parser.add_argument("--run-key", required=True)
    parser.add_argument("--duration-seconds", type=int, default=120)
    parser.add_argument("--commitment", choices=("processed", "confirmed", "finalized"), default="confirmed")
    parser.add_argument("--max-hydrations", type=int, default=300)
    ...
    if args.pumpswap_workers <= 0 or args.pumpswap_workers > 32:
        parser.error("--pumpswap-workers must be between 1 and 32")
    if args.max_concurrent_resolutions <= 0 or args.max_concurrent_resolutions > args.pumpswap_workers:
        parser.error("--max-concurrent-resolutions must be between 1 and --pumpswap-workers")
    if args.queue_size < 100 or args.queue_size > 50000:
        parser.error("--queue-size must be between 100 and 50000")

    print("Crypto Copy Trader — Unified Market Throughput Smoke v3")
    ...
    print(
        ...
        f"workers={args.pumpswap_workers} concurrent_resolutions={args.max_concurrent_resolutions} "
