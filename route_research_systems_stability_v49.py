...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Fresh v49 systems-only stability validation over the v44 provider-paced route path. "
            "It exercises acquisition/decision plumbing but intentionally prevents forward SELL collection."
        )
    )
...
def main() -> int:
    ...
    if load_route_research_outcomes(acquisition_run_key=run_key):
        raise SystemExit("v49 systems run-key already has persisted route research outcomes; use a fresh key")
    ...
    # v49 validates systems only. Force the post-systems cohort check to appear undersized so the
    # exact forward SELL collector cannot start even if the same-run systems gate passes.
    ...
    v43.v42.run_smoke_v42 = v49.run_smoke_v49
    ...
    print("\nV49 SYSTEMS STABILITY GATE")
    print(
        f"pump_prepare_workers={v49.V49_PUMP_PREPARE_WORKERS} "
        f"systems_11_of_11={systems_pass} forward_collector_started={collector_started}"
    )
    if collector_started:
        print("classification=FAIL_V49_SYSTEMS_ONLY_GUARD")
        return 2
    if not systems_pass:
        print("classification=FAIL_V49_SYSTEMS_STABILITY_PROFILE")
        print(
            "Interpretation: do not rerun v48. Keep the Flow60 hypothesis frozen and inspect the "
            "new same-run timing diagnostics before another systems change."
        )
        return 2

    print("classification=PASS_V49_SYSTEMS_STABILITY_PROFILE")
    print(
        "Interpretation: the systems-only profile passed 11/11 without collecting forward SELL "
        "labels. This does not validate the Flow60 hypothesis or any economic edge."
    )
    return 0
