...
from benchmarks.holder_ownership_native_v1.runtime_enrichment import FEATURE_ID
...
VERSION = "holder_ownership_native_v1"
PASS = "PASS_HOLDER_OWNERSHIP_NATIVE_V1"
FAIL = "FAIL_HOLDER_OWNERSHIP_NATIVE_V1"
DEFAULT_PROTOCOL = Path("benchmarks") / "holder_ownership_native_v1" / "protocol.frozen.json"
DEFAULT_CONTRACT = (
    Path("benchmarks")
...
def _validate_protocol(protocol: Mapping[str, Any]) -> None:
    ...
    if expected != actual:
        raise ValueError("native holder ownership protocol hash mismatch")
    if protocol.get("status") != "PREREGISTERED_PROSPECTIVE_DISCOVERY":
        raise ValueError("native holder ownership protocol is not preregistered discovery")
    feature_contract = protocol.get("feature_contract") or {}
    if feature_contract.get("primary_feature_id") != FEATURE_ID:
        raise ValueError("unexpected native holder primary feature")
    if feature_contract.get("expected_direction") != "negative":
        raise ValueError("native holder expected direction changed")
...
def _recompute_native_hhi(record: Mapping[str, Any]) -> float:
    ...
    if isinstance(total_supply_raw, bool) or not isinstance(total_supply_raw, int) or total_supply_raw <= 0:
        raise ValueError("invalid total_supply_raw in native holder record")
    bonding_curve = str(record.get("bonding_curve") or "").strip()
    if not bonding_curve:
        raise ValueError("missing bonding_curve in native holder record")
    owner_amounts = record.get("owner_amounts_raw")
    if not isinstance(owner_amounts, dict):
        raise ValueError("missing owner_amounts_raw in native holder record")

    values: list[float] = []
    for owner, raw_amount in owner_amounts.items():
        if str(owner) == bonding_curve:
            continue
        if isinstance(raw_amount, bool):
            raise ValueError("invalid bool owner amount")
        try:
            amount = int(raw_amount)
        except (TypeError, ValueError):
            raise ValueError("invalid owner amount")
        if amount > 0:
            values.append((float(amount) / float(total_supply_raw)) ** 2)

    if not values:
        raise ValueError("native causal holder record has no non-curve owner")
    out = sum(values)
    if not math.isfinite(out) or out < 0 or out > 1.0000000001:
        raise ValueError("native HHI outside valid range")
    return out


def _load_native_snapshot_map(run_dir: Path) -> tuple[dict[str, dict[str, Any]], dict[str, Any]]:
    route_input = _read_json(Path(run_dir) / "route-input-v2.json")
    raw_artifact = _read_json(Path(run_dir) / "holder-ownership-native-v1-evidence.json")
    raw_by_token = {
    ...
    for episode in route_input.get("episodes") or []:
        ...
        evidence = external.get("holder_ownership_native_v1") or {}
        ...
        if status == "CAUSAL_AVAILABLE":
            ...
            if str(evidence.get("excluded_market_owner") or "") != str(evidence.get("bonding_curve") or ""):
                errors.append(f"bonding_curve_exclusion_changed:{episode_key}")
            if evidence.get("graduation_observed_wall_ns") is not None:
                errors.append(f"graduation_backfilled_into_causal:{episode_key}")
            calls = [evidence.get("token_supply_call") or {}] + list(evidence.get("token_account_calls") or [])
            for call in calls:
            ...
            if not isinstance(raw, dict) or raw.get("status") != "CAUSAL_AVAILABLE":
                ...
                expected = _recompute_native_hhi(raw)
    ...
    if errors:
        raise ValueError("native holder evidence integrity failed: " + ";".join(errors[:12]))

    guardrails = raw_artifact.get("guardrails") or {}
    if guardrails.get("gmgn_dependency") is not False:
        raise ValueError("native holder raw artifact unexpectedly depends on GMGN")
...
def _validate_fresh_capture(run_dir: Path) -> dict[str, Any]:
    report = _read_json(Path(run_dir) / "simulation-report-v4-holder-ownership-native-v1.json")
    base_v4 = report.get("base_v4_report") or {}
    capture = base_v4.get("capture") or {}
    requested = base_v4.get("requested_duration_seconds")
    if report.get("classification") != "PASS_LAUNCH_BURST_HOLDER_OWNERSHIP_NATIVE_V1":
        raise ValueError("fresh native holder wrapper did not PASS")
    if int(requested or 0) != 900:
        raise ValueError("fresh native holder discovery must request exactly 900 seconds")
    ...
    if invalid_counts:
        raise ValueError(
            "fresh native holder acquisition is systems-invalid: "
            + ",".join(f"{key}={value}" for key, value in sorted(invalid_counts.items()))
        )
    if guardrails.get("gmgn_dependency") is not False:
        raise ValueError("native holder capture unexpectedly depends on GMGN")
    if guardrails.get("private_key_used") is not False:
        raise ValueError("native holder capture private-key guardrail failed")
    if guardrails.get("selector_changed") is not False:
        raise ValueError("native holder capture changed selector")
...
def run_discovery(
    *,
    ...
    holder_map, holder_integrity = _load_native_snapshot_map(fresh)
    ...
    report = {
        "type": "holder_ownership_native_report_v1",
        "version": VERSION,
        "classification": PASS,
        "decision": decision,
        ...
        "interpretation": (
            "Prospective discovery of one native holder-ownership evidence family. Higher preregistered "
            "HHI means more supply concentration among non-curve owners in a Pump pre-graduation snapshot. "
            "A sufficient sample permits mechanism/robustness review only; it cannot promote a selector."
        ),
    }

    destination = output_path or (fresh / "holder-ownership-native-v1.json")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Prospective discovery evaluator for Helius-native Holder Ownership V1"
    )
