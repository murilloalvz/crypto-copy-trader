...
VERSION = "holder_ownership_rpc_v2"
PASS = "PASS_HOLDER_OWNERSHIP_RPC_V2"
FAIL = "FAIL_HOLDER_OWNERSHIP_RPC_V2"
DEFAULT_CONTRACT = (
    Path("benchmarks")
...
def _recompute_hhi(record: Mapping[str, Any]) -> float:
    total_supply_raw = record.get("total_supply_raw")
    if isinstance(total_supply_raw, bool) or not isinstance(total_supply_raw, int) or total_supply_raw <= 0:
        raise ValueError("invalid total_supply_raw")
    bonding_curve = str(record.get("bonding_curve") or "").strip()
    owner_amounts = record.get("owner_amounts_raw")
    if not bonding_curve or not isinstance(owner_amounts, dict):
        raise ValueError("missing bonding_curve/owner_amounts_raw")
    terms = []
    for owner, raw in owner_amounts.items():
        if str(owner) == bonding_curve:
            continue
        if isinstance(raw, bool):
            raise ValueError("invalid owner amount")
        try:
            amount = int(raw)
        except (TypeError, ValueError):
            raise ValueError("invalid owner amount")
        if amount > 0:
            terms.append((float(amount) / float(total_supply_raw)) ** 2)
    if not terms:
        raise ValueError("no non-curve top20 owners")
    out = sum(terms)
    if not math.isfinite(out) or out < 0 or out > 1.0000000001:
        raise ValueError("invalid recomputed HHI")
    return out


def _load_snapshot_map(run_dir: Path) -> tuple[dict[str, dict[str, Any]], dict[str, Any]]:
    route_input = read_json(Path(run_dir) / "route-input-v2.json")
    raw_artifact = read_json(Path(run_dir) / "holder-ownership-rpc-v2-evidence.json")
    raw_by_token = {
        str(row.get("token_mint") or ""): row
        for row in raw_artifact.get("records") or []
    ...
    for episode in route_input.get("episodes") or []:
        ...
        evidence = ((snapshot.get("external_evidence") or {}).get("holder_ownership_rpc_v2") or {})
        ...
        t0_ns = int(snapshot.get("observed_t0_wall_ns") or 0)

        if status == "CAUSAL_AVAILABLE":
            request_before = evidence.get("snapshot_request_before_wall_ns")
            response_after = evidence.get("snapshot_response_after_wall_ns")
            if not isinstance(request_before, int) or request_before < t0_ns + 3_000_000_000:
                errors.append(f"snapshot_too_early:{episode_key}")
            if not isinstance(response_after, int) or response_after > cutoff_ns:
                errors.append(f"snapshot_late:{episode_key}")
            if evidence.get("denominator") != "total_supply_raw":
                errors.append(f"denominator_changed:{episode_key}")
            if evidence.get("coverage_scope") != "top20_token_accounts":
                errors.append(f"coverage_scope_changed:{episode_key}")
            if str(evidence.get("excluded_market_owner") or "") != str(evidence.get("bonding_curve") or ""):
                errors.append(f"curve_exclusion_changed:{episode_key}")
            if evidence.get("graduation_observed_wall_ns") is not None:
                errors.append(f"graduation_backfilled:{episode_key}")

            raw = raw_by_token.get(token_mint)
            if not isinstance(raw, dict) or raw.get("status") != "CAUSAL_AVAILABLE":
                errors.append(f"missing_raw_record:{episode_key}")
            else:
                ...
                if raw_feature is None or not math.isclose(raw_feature, expected, rel_tol=0, abs_tol=1e-15):
                    errors.append(f"raw_feature_mismatch:{episode_key}")
                if feature is None or not math.isclose(feature, expected, rel_tol=0, abs_tol=1e-15):
                    errors.append(f"snapshot_feature_mismatch:{episode_key}")
        elif feature is not None:
            errors.append(f"noncausal_feature_backfilled:{episode_key}")

        mapping[episode_key] = {"status": status, FEATURE_ID: feature}

    if errors:
        raise ValueError("holder ownership RPC v2 integrity failed: " + ";".join(errors[:12]))

    guards = raw_artifact.get("guardrails") or {}
    if guards.get("helius_holder_dependency") is not False:
        raise ValueError("unexpected Helius holder dependency")
    if guards.get("gmgn_holder_dependency") is not False:
        raise ValueError("unexpected GMGN holder dependency")
    return mapping, {
        ...
        "rpc_safe_host": raw_artifact.get("rpc_safe_host"),
...
def _validate_fresh_capture(run_dir: Path) -> dict[str, Any]:
    report = read_json(Path(run_dir) / "simulation-report-v4-holder-ownership-rpc-v2.json")
    base = report.get("base_v4_report") or {}
    capture = base.get("capture") or {}
    requested = base.get("requested_duration_seconds")
    if report.get("classification") != "PASS_LAUNCH_BURST_HOLDER_OWNERSHIP_RPC_V2":
        raise ValueError("fresh holder RPC v2 wrapper did not PASS")
    if int(requested or 0) != 900 or capture.get("stop_reason") != "duration_elapsed":
        raise ValueError("fresh holder RPC v2 capture duration contract failed")

    wrapper_guardrails = report.get("guardrails") or {}
    if wrapper_guardrails.get("market_ingest_provider") != "solana_public_standard_wss":
        raise ValueError("market ingest provider changed")
    if wrapper_guardrails.get("market_ingest_http_hydration") is not False:
        raise ValueError("market ingest HTTP hydration changed")
    if wrapper_guardrails.get("helius_holder_dependency") is not False:
        raise ValueError("Helius holder dependency present")
    if wrapper_guardrails.get("gmgn_holder_dependency") is not False:
        raise ValueError("GMGN holder dependency present")
    if wrapper_guardrails.get("holder_rpc_endpoint_fixed_for_run") is not True:
        raise ValueError("holder RPC endpoint was not frozen for run")
    ...
    if invalid_counts:
        raise ValueError(
            "fresh holder RPC v2 acquisition is systems-invalid: "
            + ",".join(f"{k}={v}" for k, v in sorted(invalid_counts.items()))
        )
    ...
    return {
        ...
        "holder_rpc_safe_host": wrapper_guardrails.get("holder_rpc_safe_host"),
        "helius_holder_dependency": False,
        "gmgn_holder_dependency": False,
    }
...
def run_discovery(
    *,
    ...
    holder_map, holder_integrity = _load_snapshot_map(fresh)
    ...
    primary = _association(analyzed, feature_key=FEATURE_ID, outcome_key="current_route_closed_gross_return_pct")
    secondary = _association(analyzed, feature_key=FEATURE_ID, outcome_key="current_route_closed_net_return_pct")
    copyability = _association(analyzed, feature_key=FEATURE_ID, outcome_key="current_route_usable_fixed_return_pct")
    ...
    causal = [row for row in analyzed if row.get("holder_evidence_status") == "CAUSAL_AVAILABLE"]

    report = {
        "type": "holder_ownership_rpc_report_v2",
        "version": VERSION,
        "classification": PASS,
        "decision": decision,
        ...
        "population": {
            ...
            "holder_causal_available_episode_count": len(causal),
            "holder_causal_availability_pct": 100.0 * len(causal) / len(analyzed) if analyzed else None,
        ...
        "source_integrity": {
            ...
            "late_external_evidence_backfilled": False,
    ...
    destination = Path(output_path or (fresh / "holder-ownership-rpc-v2.json"))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Prospective evaluator for Holder Ownership RPC V2")
