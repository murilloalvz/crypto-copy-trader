...
def classify_capture_v0(
    *,
    ...
    """Classify a raw capture without altering its receive clocks.
        ...
        for line_number, line in enumerate(source, start=1):
            ...
            try:
                ...
                if not isinstance(frame, dict):
                    raise ValueError("raw frame row must be a JSON object")
            ...
            for message in messages:
                ...
                if message.block_hash is not None and message.block_hash not in block_cache:
                    try:
                        resolved_row = rpc.call(
                            "eth_getBlockByHash", [message.block_hash, False]
                        )
                        block_cache[message.block_hash] = (
                            resolved_row if isinstance(resolved_row, dict) else None
                        )
                    except Exception as exc:
                        block_resolution_errors += 1
                        resolution_error = f"{type(exc).__name__}:{exc}"
                        block_cache[message.block_hash] = None
                        if len(error_samples) < MAX_ERROR_SAMPLES:
                            error_samples.append(
                                f"{message.block_hash}:{resolution_error}"
                            )
                ...
                if resolution_error is not None:
                    row["block_resolution_error"] = resolution_error
