...
def build_features_prevalidated(
    ...
    *,
    ...
    """Same frozen v1.1 feature semantics, but caller owns validation.

    This is benchmark-local until parity/capacity justify a production fast-path refactor.
    """
    ...
    known = [
        ...
        if item.token_mint == token_mint and item.observed_at <= as_of and item.chain_time <= as_of
    ]
...
class PrevalidatedBoundedMemoryRadarState:
    """Benchmark candidate: validation once at canonical adapter boundary, not per replay row."""
