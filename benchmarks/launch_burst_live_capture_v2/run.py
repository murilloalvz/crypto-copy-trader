...
CAPTURE_VERSION = "launch_burst_live_capture_v2_signal_only"
DEFAULT_DURATION_SECONDS = 900
DEFAULT_ARTIFACTS_ROOT = Path("artifacts") / CAPTURE_VERSION
ACTIVE_TIMEOUT_SECONDS = 30.0


class _BurstNoResearchKernel:
    """Preserve lifecycle/trade kernel accounting but suppress downstream trigger fan-out.

    The wrapped kernel still ingests the exact same causal lifecycle/trade observations,
    but any trigger it would emit is intentionally discarded so the shared Market-First
    chunk processor cannot create opportunity episodes, forward targets, or Research Plane
    handoffs during a Launch Burst evidence capture.
    """

    def __init__(self) -> None:
        self._inner = IndexedMarketSignalKernel()

    def ingest_lifecycle(self, observation: Any) -> Any:
        return self._inner.ingest_lifecycle(observation)

    def ingest_trade(self, observation: Any) -> None:
        self._inner.ingest_trade(observation)
        return None
...
async def _collect_rotating(
    *,
    ...
    return {
        "type": "launch_burst_rotating_acquisition_v2",
    ...
    *,
    ...
    return {
        ...
        "research_plane_disabled_by_design": (
            not state.research_errors
            and state.research_handoffs_processed == 0
            and state.first_trigger_episodes == 0
            and state.signal_boundaries_sealed == 0
        ),
...
async def _run_capture_async(
    *,
    ...
    report = {
        ...
        "gates": gates,
        "classification": (
            "PASS_LAUNCH_BURST_LIVE_CAPTURE_V2"
            if passed
            else "FAIL_LAUNCH_BURST_LIVE_CAPTURE_V2"
        ),
        "scientific_scope": {
            ...
            "no_opportunity_episode_or_forward_target_creation": True,
        },
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Launch Burst live capture v2: bounded rotating evidence, Signal Plane only"
    )
    ...
    try:
        ...
        print(f"Launch Burst live capture v2 failed: {type(exc).__name__}: {exc}", flush=True)
        return 1
    print(json.dumps(report, indent=2, sort_keys=True), flush=True)
    return 0 if report["classification"] == "PASS_LAUNCH_BURST_LIVE_CAPTURE_V2" else 2
