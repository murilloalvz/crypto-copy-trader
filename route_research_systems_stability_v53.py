...
class V53Verdict:
    ...
    v53_present: bool
    v53_resolver_ok: bool
    v53_prefetcher_ok: bool
    classification: str


def classify_v53_output(output: str) -> V53Verdict:
    systems_pass = (
    ...
    v53_resolver_ok = "v53_resolver_instance=missing" not in output
    v53_prefetcher_ok = "v53_prefetcher_instance=missing" not in output

    if collector_started:
        classification = "FAIL_V53_SYSTEMS_ONLY_GUARD"
    elif not (
        v51_present
        and v52_present
        ...
        and v53_present
        and v53_resolver_ok
        and v53_prefetcher_ok
    ):
        classification = "FAIL_V53_DIAGNOSTIC_MISSING"
    elif systems_pass and v50_attribution_ok:
        classification = "PASS_V53_OPPORTUNISTIC_PREFETCH_SYSTEMS_PROFILE"
    elif systems_pass:
        classification = "PASS_V53_OPPORTUNISTIC_PREFETCH_SYSTEMS_PROFILE_DIAGNOSTIC_INCOMPLETE"
    elif v50_attribution_ok:
        classification = "FAIL_V53_OPPORTUNISTIC_PREFETCH_SYSTEMS_PROFILE"
    else:
        classification = "FAIL_V53_OPPORTUNISTIC_PREFETCH_SYSTEMS_PROFILE_DIAGNOSTIC_INCOMPLETE"
    ...
    return V53Verdict(
        ...
        v53_resolver_ok=v53_resolver_ok,
        v53_prefetcher_ok=v53_prefetcher_ok,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Fresh v53 systems-only validation of opportunistic PumpSwap ingress prefetch. "
...
def main() -> int:
    ...
    if load_route_research_outcomes(acquisition_run_key=run_key):
        raise SystemExit(
            "v53 systems run-key already has persisted route research outcomes; use a fresh key"
        )
    ...
    verdict = classify_v53_output(capture.getvalue())

    print("\nV53 SYSTEMS STABILITY GUARD")
    print(
        ...
        f"v53_resolver_instance_ok={verdict.v53_resolver_ok} "
        f"v53_prefetcher_instance_ok={verdict.v53_prefetcher_ok} "
    ...
    if not (
        ...
        and verdict.v53_resolver_ok
        and verdict.v53_prefetcher_ok
    ):
        return 2
    if not verdict.systems_pass:
        print(
            "Interpretation: the unchanged 11-gate systems profile still failed. Keep Flow60 "
            "NOT_EVALUATED. Use this run's resolver pool-lock/capacity waits together with v50/v51/v52 "
            "clocks before any further scheduling change."
        )
        return 2
    if not verdict.v50_attribution_ok:
        print(
            "Interpretation: systems passed while exact v50 attribution was incomplete at the frozen "
            "deadline. Preserve the systems PASS independently; v53 wait-stage telemetry remains "
            "diagnostic only."
        )
        return 0

    print(
        "Interpretation: the unchanged systems gate passed with opportunistic-only prefetch, v52 "
        "deadline semantics and complete causal-clock attribution. This is systems evidence only; "
