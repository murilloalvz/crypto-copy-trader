...
def main() -> None:
    parser = argparse.ArgumentParser(description="Unified market latency smoke v6")
    ...
    if args.pump_batch_size <= 1:
        parser.error("pump-batch-size must be greater than 1 for v6 microbatch mode")
    ...
    print("Crypto Copy Trader — Unified Market Latency Smoke v6")
    ...
    print(
        "v6 changes only Pump persistence scheduling: one ordered SQLite writer amortizes commits "
        "across bounded microbatches. Detector thresholds, causal clocks, radar ordering, PumpSwap "
        "configuration, and replay semantics are unchanged. Execution/risk providers are not called."
    )
