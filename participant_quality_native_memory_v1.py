...
VERSION = "participant_quality_native_memory_v1_role_normalized_epoch"
...
def _participant_feature_for_episode(
    *,
    run_key: str,
    episode_key: str,
    allowed_history_run_keys: tuple[str, ...],
) -> dict[str, Any]:
    ...
    history = load_route_research_wallet_history_v0(
        ...
        allowed_acquisition_run_keys=allowed_history_run_keys,
    )
        ...
        rows = [
            {
                ...
                **_participant_feature_for_episode(
                    run_key=run_key,
                    episode_key=episode_key,
                    allowed_history_run_keys=run_keys[: index - 1],
                ),
            }
...
def run_memory_build(
    *,
    ...
    artifact_root: Path = Path(
        "artifacts/participant_quality_native_memory_v1"
    ),
) -> dict[str, Any]:
    ...
    report = {
        ...
        "history_source": (
            "route_research_decisions_and_outcomes_only_"
            "restricted_to_prior_valid_role_normalized_memory_cohorts"
        ),
        "run_reports": run_reports,
        ...
        "input_semantics": (
            "PumpSwap market observations normalized to the non-reference "
            "opportunity asset before Signal Plane detection; history restricted "
            "to prior cohorts from this clean post-fix memory epoch."
        ),
        "interpretation": (
...
def main() -> int:
    ...
    print("\nCrypto Copy Trader — Participant Quality Native Memory V1")
